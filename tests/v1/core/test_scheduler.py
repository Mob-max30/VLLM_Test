...
def test_check_stop_min_tokens():
    ...
    # Test case 5: structured-output requests may stop early on a normal stop
    # token, but still respect ignore_eos.
    for ignore_eos, should_stop in ((False, True), (True, False)):
        ...
        sampling_params_structured.update_from_generation_config({}, EOS_TOKEN_ID)
        request_structured = Request(
            request_id=f"structured-{ignore_eos}",
            prompt_token_ids=[0, 1, 2],
            sampling_params=sampling_params_structured,
            pooling_params=None,
        )
        request_structured.structured_output_request = Mock()
        request_structured.append_output_token_ids([EOS_TOKEN_ID])
        result = check_stop(request_structured, max_model_len=100)
        assert result is should_stop
        if should_stop:
            assert request_structured.status == RequestStatus.FINISHED_STOPPED

    # Test case 6: min_tokens met, should stop on stop token
...
@pytest.mark.parametrize(
    ("new_token_ids", "expected_advance_token_ids"),
    [([EOS_TOKEN_ID], []), ([42, EOS_TOKEN_ID], [42])],
)
def test_stopped_structured_output_request_does_not_advance_stop_token(
    new_token_ids: list[int], expected_advance_token_ids: list[int]
):
    scheduler = object.__new__(Scheduler)
    sampling_params = SamplingParams(ignore_eos=False, max_tokens=4)
    sampling_params.update_from_generation_config({}, EOS_TOKEN_ID)
    ...
    scheduler.structured_output_manager.should_advance.return_value = True
    scheduler.structured_output_manager.trim_reasoning_for_advance.side_effect = (
        lambda request, token_ids: token_ids
    )
    scheduler.requests = {request.request_id: request}
    scheduler.running = [request]
    scheduler.waiting = Mock()
    ...
    output = SchedulerOutput(
        scheduled_new_reqs=[],
        scheduled_cached_reqs=CachedRequestData.make_empty(),
        num_scheduled_tokens={request.request_id: len(new_token_ids)},
        total_num_scheduled_tokens=len(new_token_ids),
        scheduled_encoder_inputs={},
        scheduled_spec_decode_tokens={},
        num_common_prefix_blocks=[],
        finished_req_ids=set(),
    ...
    scheduler.structured_output_manager.should_advance.assert_called_once_with(request)
    if expected_advance_token_ids:
        request.structured_output_request.grammar.accept_tokens.assert_called_once_with(
            request.request_id, expected_advance_token_ids
        )
    else:
    ...
    engine_core_output = engine_core_outputs[0].outputs[0]
    assert engine_core_output.request_id == request.request_id
    assert engine_core_output.new_token_ids == new_token_ids
