...
def prepare_nvfp4_moe_layer_for_flashinfer_cutedsl(
    ...
    Interleaves w13 gate/linear rows and swizzles the weight block scales.
    """
    # Global scaling factors (same as other FlashInfer backends).
    num_experts = w13.shape[0]
    enable_eplb = layer.moe_config.moe_parallel_config.enable_eplb
    a13_scale = amax_for_moe_activation_quant(a13_scale, enable_eplb).expand(
        num_experts
    )
    a2_scale = amax_for_moe_activation_quant(a2_scale, enable_eplb).expand(num_experts)
...
def prepare_nvfp4_moe_layer_for_fi_or_cutlass(
    ...
    if is_global_sf_supported_for_nvfp4_backend(backend):
        ...
        a2_scale = amax_for_moe_activation_quant(a2_scale, enable_eplb).expand(
            num_experts
        )
    else:
