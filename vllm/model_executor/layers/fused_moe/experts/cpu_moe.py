...
def prepare_int8_moe_layer_for_arm(
    w13: torch.Tensor,
    w2: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """NEON-prepack INT8 MoE weights for the Arm SMMLA kernel."""
    return (
...
class CPUExpertsInt8Arm(mk.FusedMoEExpertsMonolithic):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        w13, w2 = prepare_int8_moe_layer_for_arm(layer.w13_weight, layer.w2_weight)
        replace_parameter(layer, "w13_weight", w13)
        replace_parameter(layer, "w2_weight", w2)
