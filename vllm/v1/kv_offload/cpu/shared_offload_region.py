...
class SharedOffloadRegion:
    ...
    def create_next_shared_view(self, area_size: int) -> torch.Tensor:
        """Allocate a strided int8 view spanning all workers' slots for one
        canonical tensor (canonical fragment-major layout). Must be called
        once per canonical tensor, instead of create_next_view."""
